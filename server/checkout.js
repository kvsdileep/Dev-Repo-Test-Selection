...
function applyPromo(lines, promoCode) {
  ...
  const hasEligible = lines.some(line => line.category === promo.category);
  if (!hasEligible) {
    return { discount: 0, promo: null, message: `${code} requires ${promo.category} items in your cart` };
  }

  const cartCents = lines.reduce((sum, line) => sum + toCents(line.lineTotal), 0);
  const discount = fromCents(Math.round(cartCents * promo.rate));
